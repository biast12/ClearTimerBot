...
class ScheduleExpressionParser:
    ...
    FRACTION_TIME_PATTERN = re.compile(r"^(\d+)/(\d+)\s+(\d{1,2}:\d{2})(?:\s+([A-Z][\w+-]*))?$")
    ...
    def _parse_fractional_time_schedule(self, match: re.Match, server_id: str = None) -> Tuple[CronTrigger, datetime]:
        ...
        if numerator != 1:
            raise ScheduleParseError(f"Only unit fractions (1/n) are supported. Got {numerator}/{denominator}")
        
        # Check if denominator is a factor of 24
        if 24 % denominator != 0:
            raise ScheduleParseError(f"Denominator {denominator} must be a factor of 24 (valid: 1, 2, 3, 4, 6, 8, 12, 24)")
        ...
        for run_hour in sorted(run_hours):
            potential_time = now.replace(hour=run_hour, minute=minute, second=0, microsecond=0)
        ...
        if next_run is None:
            next_run = now.replace(hour=run_hours[0], minute=minute, second=0, microsecond=0)
