...
class AdminListView(discord.ui.LayoutView):
    ...
    def __init__(self, admins: set, bot, translator):
        ...
        total_text = translator.get(
            "commands.owner.admin.list.total", count=admin_count, admin_text=admin_text
        )
...
class AdminAddSuccessView(discord.ui.LayoutView):
    ...
    def __init__(self, username: str, user_id: str, translator):
        ...
        description = translator.get(
            "commands.owner.admin.add.success_description", username=username
        )
        user_id_text = translator.get(
            "commands.owner.admin.add.success_user_id", user_id=user_id
        )
        mention_text = translator.get(
            "commands.owner.admin.add.success_mention", user_id=user_id
        )
...
class AdminAddAlreadyView(discord.ui.LayoutView):
    ...
    def __init__(self, user_id: str, translator):
        ...
        description = translator.get(
            "commands.owner.admin.add.already_description", user_id=user_id
        )
        content = f"⚠️ **{title}**\n\n{description}\nMention: <@{user_id}>"
...
class AdminRemoveSuccessView(discord.ui.LayoutView):
    ...
    def __init__(self, user_id: str, translator):
        ...
        description = translator.get(
            "commands.owner.admin.remove.success_description", user_id=user_id
        )
        previous = translator.get(
            "commands.owner.admin.remove.success_previous", user_id=user_id
        )
...
class AdminRemoveNotFoundView(discord.ui.LayoutView):
    ...
    def __init__(self, user_id: str, translator):
        ...
        description = translator.get(
            "commands.owner.admin.remove.not_found_description", user_id=user_id
        )
...
class ShardReloadSingleView(discord.ui.LayoutView):
    ...
    def __init__(self, shard_id: int, translator):
        ...
        description = translator.get(
            "commands.owner.shard.reload.single_description", shard_id=shard_id
        )
...
class ShardNotShardedView(discord.ui.LayoutView):
    ...
    def __init__(self, translator):
        ...
        description = translator.get(
            "commands.owner.shard.reload.not_sharded_description"
        )
...
class ShardInvalidIdView(discord.ui.LayoutView):
    ...
    def __init__(self, shard_id: int, max_shard: int, translator):
        ...
        description = translator.get(
            "commands.owner.shard.reload.invalid_id_description", shard_id=shard_id
        )
        range_text = translator.get(
            "commands.owner.shard.reload.invalid_id_range", max_shard=max_shard - 1
        )
...
class ShardReloadSignalView(discord.ui.LayoutView):
    ...
    def __init__(self, shard_id: int, translator):
        ...
        description = translator.get(
            "commands.owner.shard.reload.signal_sent_description", shard_id=shard_id
        )
...
class ShardReloadFailedView(discord.ui.LayoutView):
    ...
    def __init__(self, error: str, translator):
        ...
        error_text = translator.get(
            "commands.owner.shard.reload.failed_error", error=error
        )
...
class ShardStatusCompleteView(discord.ui.LayoutView):
    ...
    def __init__(self, bot, translator):
        ...
        if bot.shard_id is not None:
            content += (
                translator.get(
                    "commands.owner.shard.status.shard_info",
            ...
            content += (
                translator.get(
                    "commands.owner.shard.status.total_shards", count=bot.shard_count
                )
                + "\n"
            )
            content += (
                translator.get(
                    "commands.owner.shard.status.guilds_on_shard", count=len(bot.guilds)
                )
                + "\n"
            )
            ...
            try:
                ...
                latency = (
                    shard.latency
                    if shard and shard.latency is not None
                    else bot.latency
                )
            ...
            content += (
                translator.get(
                    "commands.owner.shard.status.latency",
            ...
            content += (
                translator.get(
                    "commands.owner.shard.status.total_guilds", count=len(bot.guilds)
                )
                + "\n"
            )
            content += (
        ...
        pid = os.getpid()
        content += (
            translator.get("commands.owner.shard.status.process_id", pid=pid) + "\n\n"
        )

        available_commands = translator.get(
            "commands.owner.shard.status.available_commands"
        )
        cmd_reload_all = translator.get("commands.owner.shard.status.cmd_reload_all")
        cmd_reload_single = translator.get(
            "commands.owner.shard.status.cmd_reload_single"
        )
