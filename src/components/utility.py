...
class HelpView(discord.ui.LayoutView):
    ...
    def __init__(self):
        ...
        content = (
            ...
            "`/sub add <timer> [channel] [target]` - Subscribe channel with optional ignored target\n"
            "`/sub remove [channel]` - Unsubscribe a channel\n"
            "`/sub update <timer> [channel] [target]` - Update timer with optional ignored target\n"
            "`/sub info [channel]` - View subscription details\n"
            "`/sub ignore <target> [channel]` - Toggle message or user ignore status\n"
