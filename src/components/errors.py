...
class ErrorView(discord.ui.LayoutView):
    ...
    def __init__(
        self, title: str, description: str, translator, error_id: Optional[str] = None
    ):
        super().__init__()
...
class MissedClearView(discord.ui.LayoutView):
    ...
    def __init__(
        self, translator, channel: discord.TextChannel, message_clearing_service, bot
    ):
        super().__init__()
    ...
    async def clear_now_callback(self, interaction: discord.Interaction):
        ...
        is_valid, error_msg, _ = await validator.validate_command(
            interaction, target_channel=self.channel, checks=checks
        )

        if not is_valid:
            await validator.send_validation_error(
                interaction, error_msg, ephemeral=True
            )
            return
