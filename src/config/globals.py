...
class GlobalConfig:
    ...
    missed_clear_notification_timeout: float = (
        0.0  # Delete missed clear notifications after 60 seconds
    )
    ...
    def __post_init__(self):
        ...
        self.bot_color = (
            int(os.getenv("BOT_COLOR", str(self.bot_color)), 16)
            if os.getenv("BOT_COLOR")
            else self.bot_color
        )
        self.error_color = (
            int(os.getenv("ERROR_COLOR", str(self.error_color)), 16)
            if os.getenv("ERROR_COLOR")
            else self.error_color
        )
        self.success_color = (
            int(os.getenv("SUCCESS_COLOR", str(self.success_color)), 16)
            if os.getenv("SUCCESS_COLOR")
            else self.success_color
        )
        self.warning_color = (
            int(os.getenv("WARNING_COLOR", str(self.warning_color)), 16)
            if os.getenv("WARNING_COLOR")
        ...
        # Rate Limiting
        self.max_clears_per_minute = int(
            os.getenv("MAX_CLEARS_PER_MINUTE", str(self.max_clears_per_minute))
        )
        self.max_subscriptions_per_server = int(
            os.getenv(
                "MAX_SUBSCRIPTIONS_PER_SERVER", str(self.max_subscriptions_per_server)
            )
        )
        ...
        # Message Settings
        self.ephemeral_error_timeout = float(
            os.getenv("EPHEMERAL_ERROR_TIMEOUT", str(self.ephemeral_error_timeout))
        )
        ...
        # Database Settings
        self.removed_servers_retention_days = int(
            os.getenv(
                "REMOVED_SERVERS_RETENTION_DAYS",
                str(self.removed_servers_retention_days),
            )
        )
        self.error_logs_retention_days = int(
            os.getenv("ERROR_LOGS_RETENTION_DAYS", str(self.error_logs_retention_days))
        )
        ...
        self.terms_of_service_url = os.getenv(
            "TERMS_OF_SERVICE_URL", self.terms_of_service_url
        )
        self.privacy_policy_url = os.getenv(
