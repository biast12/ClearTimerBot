...
class ClearTimerBot(commands.Bot):
    ...
    async def rotate_activity(self):
        ...
        activity = discord.CustomActivity(
            name=f"🧹 Cleaning up {total_subscriptions} channels{dots}"
        )
    ...
    async def on_ready(self) -> None:
        if self.shard_id is not None:
            logger.info(
                LogArea.STARTUP,
                f"Shard {self.shard_id}/{self.shard_count - 1} ready: {self.user} (ID: {self.user.id})",
            )
            logger.info(
                LogArea.STARTUP,
                f"Shard {self.shard_id} connected to {len(self.guilds)} guilds",
            )
        else:
        ...
        try:
            ...
            if self.shard_id is not None:
                logger.info(
                    LogArea.STARTUP, f"Shard {self.shard_id} initialization complete!"
                )
            else:
            ...
            logger.critical(
                LogArea.STARTUP, f"Bot initialization failed! Error ID: {error_id}"
            )
    ...
    async def load_commands(self) -> None:
        ...
        for module in command_modules:
            try:
                ...
                error_id = await logger.log_error(
                    LogArea.STARTUP, f"Failed to load extension {module}", exception=e
                )
                logger.error(
                    LogArea.STARTUP,
                    f"Failed to load extension {module}. Error ID: {error_id}",
                )
        ...
        if self.config.guild_id:
            try:
                ...
                error_id = await logger.log_error(
                    LogArea.STARTUP, "Failed to load admin commands", exception=e
                )
                logger.error(
                    LogArea.STARTUP,
                    f"Failed to load admin commands. Error ID: {error_id}",
                )
        ...
        if self.config.owner_id and self.config.guild_id:
            try:
                ...
                error_id = await logger.log_error(
                    LogArea.STARTUP, "Failed to load owner commands", exception=e
                )
                logger.error(
                    LogArea.STARTUP,
                    f"Failed to load owner commands. Error ID: {error_id}",
                )

    async def close(self) -> None:
        if getattr(self, "restart_requested", False):
            logger.info(
                LogArea.STARTUP, "Restart requested, initiating graceful shutdown..."
            )
    ...
    async def on_guild_join(self, guild: discord.Guild) -> None:
        ...
        if removed_doc:
            ...
            logger.info(
                LogArea.DISCORD, f"Bot rejoined server: {guild.name} (ID: {server_id})"
            )
            ...
            logger.info(
                LogArea.DISCORD,
                f"Bot joined new server: {guild.name} (ID: {server_id})",
            )
        ...
        logger.info(
            LogArea.DISCORD, f"Set language for {guild.name}: {detected_language}"
        )
    ...
    async def on_guild_remove(self, guild: discord.Guild) -> None:
        ...
        logger.info(
            LogArea.DISCORD, f"Bot removed from server: {guild.name} (ID: {server_id})"
        )
    ...
    async def _sync_server_cleanup_status(self) -> None:
        ...
        for removal_doc in servers_to_mark_removed:
            ...
            if not existing:
                ...
                logger.info(
                    LogArea.CLEANUP,
                    f"Marked server {removal_doc['server_name']} (ID: {server_id}) as removed (bot not in server)",
                )
        ...
        for removed_doc in removed_servers:
            ...
            if server_id in current_guild_ids:
                ...
                logger.info(
                    LogArea.CLEANUP,
                    f"Removed server {guild_name} (ID: {server_id}) from removal tracking (bot is in server)",
                )
    ...
    async def _cleanup_deleted_channels(self) -> None:
        ...
        for server_id, server in all_servers.items():
            ...
            for channel_id in subscribed_channels:
                ...
                if not channel:
                    await self.data_service.remove_channel_subscription(
    ...
    async def on_guild_channel_delete(self, channel: discord.abc.GuildChannel) -> None:
        ...
        if server and channel_id in server.channels:
            if await self.data_service.remove_channel_subscription(
                server_id, channel_id
            ):
