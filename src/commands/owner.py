...
class OwnerCommands(
    ...
    async def admin_add(self, interaction: discord.Interaction, user_id: str):
        ...
        try:
                ...
                try:
                    ...
                    if not user:
                        ...
                        await self.data_service._cache.set(
                            cache_key, user, cache_level="warm", ttl=1800
                        )

                    username = str(user)
                except Exception as e:
                    logger.warning(
                        LogArea.COMMANDS, f"Failed to fetch user {user_id}: {e}"
                    )
            ...
            view = ErrorView(
                "❌ **Failed to Add Admin**", f"An error occurred: {str(e)}", translator
            )
            await interaction.followup.send(view=view)
    ...
    @app_commands.describe(user_id="Discord user ID to remove from admins")
    ...
    async def shard_reload(
        self, interaction: discord.Interaction, shard_id: Optional[int] = None
    ):
        await interaction.response.defer(ephemeral=True)
        ...
        try:
            ...
            if shard_id is not None:
                if self.bot.shard_count and (
                    shard_id < 0 or shard_id >= self.bot.shard_count
                ):
                    from src.components.owner import ShardInvalidIdView

                    view = ShardInvalidIdView(
                        shard_id, self.bot.shard_count, translator
                    )
                    await interaction.followup.send(view=view)
                    return
                ...
                if self.bot.shard_id == shard_id:
                    logger.info(
                        LogArea.COMMANDS,
                        f"Reloading current shard {shard_id} (self-restart)",
                    )
                    ...
                    logger.info(
                        LogArea.COMMANDS, f"Reload requested for shard {shard_id}"
                    )
