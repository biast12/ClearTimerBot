...
class AdminCommands(
        ...
        if not (is_owner or is_admin):
            ...
            if interaction.guild:
                translator = await get_translator(
                    str(interaction.guild.id), self.data_service
                )
    ...
    @app_commands.describe(server_id="Server ID to get specific stats for (optional)")
    async def stats(self, interaction: discord.Interaction, server_id: str = None):
        ...
        if server_id:
            ...
            if server_errors is None:
                server_errors = await errors_collection.count_documents(
                    {"server_id": server_id}
                )
                await self.data_service._cache.set(
                    cache_key, server_errors, cache_level="memory", ttl=300
                )
            ...
            if error_count is None:
                ...
                await self.data_service._cache.set(
                    cache_key, error_count, cache_level="memory", ttl=300
                )
    ...
    @app_commands.describe(
        server_id="Server ID to blacklist", reason="Reason for blacklisting (optional)"
    )
    async def blacklist_add(
        ...
        if await self.data_service.add_to_blacklist(
            server_id, server_name, reason=reason, blacklisted_by=blacklisted_by
        ):
            ...
            if server:
                for channel_id in list(server.channels.keys()):
                    self.scheduler_service.remove_channel_clear_job(
    ...
    async def recache(self, interaction: discord.Interaction):
        ...
        try:
            ...
            channels_count = sum(
                len(s.channels) for s in self.data_service._servers_cache.values()
            )
    ...
    async def error_clear(self, interaction: discord.Interaction):
        ...
        try:
            ...
            error_id = await logger.log_error(
                LogArea.DATABASE, "Failed to clear errors from database", exception=e
            )
    ...
    async def force_remove_channel(self, interaction: discord.Interaction, id: str):
        ...
        for server_id, server in servers.items():
            if id in server.channels:
                ...
                view = ForceUnsubSuccessView(
                    "channel", id, translator, server_id=server_id
                )
                await interaction.followup.send(view=view)
                return
