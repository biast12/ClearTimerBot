...
class SubscriptionCommands(commands.Cog):
    ...
    def add_context_menus(self) -> None:
        ...
        async def ignore_message_context(
    ...
    async def handle_ignore_message_context(
        ...
        if not server or channel_id not in server.channels:
            await interaction.response.send_message(
                translator.get(
                    "validation.not_subscribed", channel=message.channel.mention
                ),
                ephemeral=True,
            )
            return
    ...
    async def handle_ignore_user_context(
        ...
        if not isinstance(channel, discord.TextChannel):
            await interaction.response.send_message(
                translator.get("validation.invalid_channel"), ephemeral=True
            )
            return
    ...
    async def subscription_add(
        ...
        # Parse timer
        try:
            trigger, next_run_time = self.schedule_parser.parse_schedule_expression(
                timer, server_id
            )
        ...
        if view:
            ...
            timer_view = TimerViewMessage(
                channel, timer_to_store, next_run_time, translator
            )
        ...
        from src.components.subscription import (
            SubscriptionSuccessView,
            SubscriptionSuccessWithMultipleIgnoresView,
        )

        if len(added_targets) == 1:
            success_view = SubscriptionSuccessView(
                channel,
                timer_to_store,
                next_run_time,
                translator,
            ...
            success_view = SubscriptionSuccessWithMultipleIgnoresView(
                channel, timer_to_store, next_run_time, added_targets, translator
            )
        else:
            success_view = SubscriptionSuccessView(
                channel, timer_to_store, next_run_time, translator, None, None
            )
                    ...
                    try:
                        ...
                        if not message:
                            ...
                            await self.data_service._cache.set(
                                cache_key, message, cache_level="memory", ttl=60
                            )
    ...
    async def subscription_list(
        ...
        view = SubscriptionListView(
            interaction.guild, server.channels, self.scheduler_service, translator
        )
        await interaction.response.send_message(view=view)
    ...
    async def subscription_update(
        ...
        if server and channel_id in server.channels:
            current_ignored_messages = list(
                server.channels[channel_id].ignored.messages
            )
        ...
        if server:
            ...
            if view:
                    ...
                    try:
                        ...
                        if not old_message:
                            old_message = await channel.fetch_message(
                                int(old_view_message_id)
                            )
                            # Cache briefly since we're about to delete it
                            await self.data_service._cache.set(
                                cache_key, old_message, cache_level="memory", ttl=60
                            )
                ...
                try:
                    ...
                    logger.debug(
                        LogArea.COMMANDS,
                        f"Could not update view message {old_view_message_id}: {e}",
                    )
        ...
        from src.components.subscription import (
            UpdateSuccessView,
            UpdateSuccessWithMultipleIgnoresView,
        )

        if len(added_targets) == 1:
            success_view = UpdateSuccessView(
                channel,
                timer_to_store,
                next_run_time,
                translator,
            ...
            # Use a new view for multiple targets
            success_view = UpdateSuccessWithMultipleIgnoresView(
                channel, timer_to_store, next_run_time, added_targets, translator
            )
        else:
            success_view = UpdateSuccessView(
    ...
    async def subscription_clear_now(
        ...
        message_service = MessageService(
            self.bot.data_service, self.bot.scheduler_service
        )
        deleted_count = await message_service._perform_message_deletion(
            channel, set(ignored_messages), set(ignored_users)
        )
    ...
    async def subscription_skip_next(
        ...
        if next_run_time:
            ...
            new_next_run_time = job.trigger.get_next_fire_time(
                next_run_time, next_run_time
            )
            self.scheduler_service.scheduler.modify_job(
                job.id, next_run_time=new_next_run_time
            )
