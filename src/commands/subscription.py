...
class SubscriptionCommands(commands.Cog):
    ...
    @app_commands.command(name="sub", description="Subscribe a channel to automatic message deletion")
    ...
    async def subscribe(
        ...
        if not server:
            server = await self.data_service.add_server(server_id, interaction.guild.name)
    ...
    @app_commands.command(name="unsub", description="Unsubscribe a channel from automatic message deletion")
    @app_commands.describe(
