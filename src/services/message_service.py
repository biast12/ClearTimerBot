...
class MessageService:
    ...
    async def clear_channel_messages(self, channel: discord.TextChannel) -> None:
        print(f"Starting message clear for channel {channel.id} in guild {channel.guild.id}")
            ...
            if messages_to_delete:
                ...
                for i in range(0, len(messages_to_delete), 100):
                    ...
                    try:
                        ...
                        print(f"Bulk delete failed: {e}, falling back to individual deletion")
                        for msg in batch:
                            try:
                                await msg.delete()
                                deleted_count += 1
    ...
    async def _update_next_run_time(self, channel: discord.TextChannel) -> None:
        ...
        if server and channel_id in server.channels:
            server.channels[channel_id].next_run_time = next_run_time.astimezone(pytz.UTC)
            await self.data_service.save_servers()
    
    async def notify_missed_clear(self, channel: discord.TextChannel, job_id: str) -> None:
        try:
            embed = discord.Embed(
                title="⚠️ Missed Clear Notification",
                description=(
                    f"A scheduled message clear was missed for this channel.\n"
                    f"The timer has been rescheduled."
                ),
