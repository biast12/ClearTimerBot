...
class ShardManager:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(
                    ...
                    if resp.status == 200:
                        ...
                        logger.info(
                            LogArea.STARTUP,
                            f"Discord recommends {shard_count} shard(s)",
                        )
                        return shard_count
                    else:
                        logger.warning(
                            LogArea.STARTUP,
                            f"Failed to get shard recommendation: HTTP {resp.status}",
                        )
                        return 1
    ...
    async def launch_shard(
        self, shard_id: int, shard_count: int, auto_restart: bool = True
    ...
    async def monitor_shard(self, shard_id: int, shard_count: int) -> None:
        while shard_id in self.processes:
            ...
            if process:
                ...
                if return_code == 99:
                    logger.info(
                        LogArea.STARTUP, f"Shard {shard_id} requested manager restart"
                    )
                ...
                logger.warning(
                    LogArea.STARTUP,
                    f"Shard {shard_id} crashed with exit code {return_code}",
                )
                ...
                if restart_count >= self.max_restart_attempts:
                    logger.error(
                        LogArea.STARTUP,
                        f"Shard {shard_id} exceeded max restart attempts ({self.max_restart_attempts})",
                    )
                    break

                logger.info(
                    LogArea.STARTUP,
                    f"Restarting shard {shard_id} in {self.restart_cooldown} seconds...",
                )
                await asyncio.sleep(self.restart_cooldown)
                self.shard_restart_counts[shard_id] = restart_count + 1
                logger.info(
                    LogArea.STARTUP,
                    f"Restarting shard {shard_id} (attempt {restart_count + 1}/{self.max_restart_attempts})",
                )
    ...
    async def _shutdown_all_shards(self) -> None:
        ...
        if self.processes:
            await asyncio.gather(
                *[p.wait() for p in self.processes.values()], return_exceptions=True
            )
    ...
    async def run(self) -> bool:
        try:
            if self.shard_count is None:
                ...
                logger.info(
                    LogArea.STARTUP, f"Auto-detected shard count: {self.shard_count}"
                )
            else:
                logger.info(
                    LogArea.STARTUP, f"Using manual shard count: {self.shard_count}"
                )
            ...
            logger.info(
                LogArea.STARTUP,
                f"Launching {len(self.shard_ids)} shard(s): {self.shard_ids}",
            )
            ...
            restart_task = asyncio.create_task(self._wait_for_restart())
                ...
                [monitor_task, restart_task], return_when=asyncio.FIRST_COMPLETED
            )
            ...
            if self.should_restart:
                logger.info(
                    LogArea.STARTUP,
                    "Manager restart requested, shutting down all shards...",
                )
            ...
            logger.info(
                LogArea.STARTUP, "Received shutdown signal, stopping all shards..."
            )
            for shard_id, process in self.processes.items():
                process.terminate()
            await asyncio.gather(
...
async def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="ClearTimer Bot with automatic sharding"
    )
    parser.add_argument(
        "--shards",
        type=int,
        help="Total number of shards (auto-detect if not specified)",
    )
    parser.add_argument(
        "--shard-ids",
        type=str,
        help="Comma-separated list of shard IDs to run (all if not specified)",
    )
    parser.add_argument(
        "--force-shards",
        type=int,
        help="Force a specific number of shards (overrides auto-detection)",
    )
    parser.add_argument(
        "--max-restarts",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=30,
        help="Cooldown between shard restarts in seconds (default: 30)",
    )
    ...
    while True:
        try:
            ...
            if args.max_restarts:
                manager.max_restart_attempts = args.max_restarts // 3
            ...
            if needs_restart:
                ...
                if restart_count > max_restarts:
                    logger.error(
                        LogArea.STARTUP,
                        f"Maximum restart attempts ({max_restarts}) exceeded",
                    )
                    break

                logger.info(
                    LogArea.STARTUP,
                    f"=== RESTARTING SHARD MANAGER (attempt {restart_count}/{max_restarts}) ===",
                )
                await asyncio.sleep(5)
                continue
            else:
            ...
            logger.info(
                LogArea.STARTUP, "Received keyboard interrupt, shutting down..."
            )
            break
        except Exception as e:
            import traceback
            ...
            if restart_count > max_restarts:
                logger.error(
                    LogArea.STARTUP,
                    f"Maximum restart attempts ({max_restarts}) exceeded after error",
                )
                break
            logger.info(
                LogArea.STARTUP,
                f"Attempting restart after error (attempt {restart_count}/{max_restarts})...",
            )
